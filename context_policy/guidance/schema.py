...
class RepoGuidance:
    ...
    The *only* thing the tuning loop edits.  Everything else
    (system prompt, runner settings, harness) stays frozen.
    """
    ...
    def render(self) -> str:
        """Join lines into the guidance text block."""
        return "\n".join(self.lines)
    ...
    def copy(self, *, version: int | None = None, lines: list[str] | None = None) -> RepoGuidance:
        """Return a shallow copy, optionally replacing version/lines."""
        return RepoGuidance(
            repo=self.repo,
            commit=self.commit,
