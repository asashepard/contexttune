...
Detects pytest config, test directories, conftest files, and fixtures.
"""
...
from context_policy.probes.schema import TestInfo
...
def _find_test_dirs(repo_dir: Path) -> list[str]:
    """Find directories that look like test roots."""
    found: list[str] = []
...
def _detect_test_command(repo_dir: Path) -> str:
    """Best-effort guess of the test command."""
    if (repo_dir / "pytest.ini").exists():
        return "pytest"
    if (repo_dir / "pyproject.toml").exists():
        try:
            text = (repo_dir / "pyproject.toml").read_text(
    ...
    if (repo_dir / "setup.cfg").exists():
        try:
            text = (repo_dir / "setup.cfg").read_text(
                encoding="utf-8", errors="ignore",
            )
            if "[tool:pytest]" in text:
                return "pytest"
...
def _detect_fixtures(repo_dir: Path, conftest_paths: list[str]) -> list[str]:
    """Extract pytest fixture names from conftest files."""
    fixtures: list[str] = []
    for rel in conftest_paths:
        ...
        for line in text.splitlines():
            ...
            if stripped.startswith("@pytest.fixture") or stripped.startswith("@fixture"):
                # Next non-decorator line should be the def
                continue
            if stripped.startswith("def ") and "()" not in stripped:
                # This is approximate — good enough for convention detection
...
def detect_tests(repo_dir: Path) -> TestInfo:
    """Detect test infrastructure for the repository.

    Args:
        repo_dir: Root of the checked-out repository.

    Returns:
        ``TestInfo`` with test command, dirs, conftest paths, and fixtures.
    """
