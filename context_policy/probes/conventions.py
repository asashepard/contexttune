...
Detects docstring format, type hint prevalence, import ordering,
and linter configurations from AST and config files.
"""
...
from context_policy.probes.schema import Conventions
from context_policy.utils.ignore import should_ignore_dir, should_ignore_file

MAX_DIRECTORIES = 12

# Config files that indicate linter/formatter usage
...
# pyproject.toml sections that indicate tool configuration
...
def _detect_docstring_style(trees: dict[str, Any], source_map: dict[str, bytes]) -> str:
    """Sample docstrings from functions/classes to detect style."""
    google_count = 0
    numpy_count = 0
    sphinx_count = 0
    total = 0
    ...
    for rel_path, tree in list(trees.items())[:30]:
        ...
        for node in tree.root_node.children:
            if node.type in ("function_definition", "class_definition", "decorated_definition"):
                ...
                if body is None:
                    # For decorated_definition, dig into the inner definition
...
def _detect_type_hint_prevalence(
    trees: dict[str, Any], source_map: dict[str, bytes],
) -> float:
    """Estimate fraction of functions that have any type annotations."""
    annotated = 0
    total = 0
...
def _detect_linter_configs(repo_dir: Path) -> list[str]:
    """Find linter/formatter config files in repo root."""
    found: list[str] = []
    try:
    ...
    # Check setup.cfg for tool sections
...
def detect_conventions(
    ...
    """Detect coding conventions for the repository.
    ...
    Returns:
        ``Conventions`` with detected patterns.
    """
