...
Clusters files that share importers — purely structural, no git history.
Two files that are imported by >=2 of the same consumers get clustered
together.  Score = number of shared importers.
"""
from __future__ import annotations

from collections import defaultdict

from context_policy.probes.schema import (
    ClusterResults,
    FileCluster,
    ImportChain,
    ImportGraph,
...
def _compute_co_import_scores(
    imported_by: dict[str, list[str]],
) -> dict[tuple[str, str], int]:
    """Compute pairwise co-import scores.

    For every pair of files (A, B), the score is the number of files
    that import *both* A and B.

    Only pairs with score >= 2 are returned.
    """
    # Invert: for each importer, which files does it import?
    ...
    # Count shared importers for each pair
    scores: dict[tuple[str, str], int] = {}
    # For each importer, all pairs of its targets share that importer
...
def _greedy_cluster(
    ...
    """Greedy agglomerative clustering from co-import scores.

    Processes edges in descending score order.  Each file is assigned
    ...
    for (a, b), score in sorted(scores.items(), key=lambda x: -x[1]):
        ...
        if ca is not None and cb is not None:
            ...
            # Don't merge existing clusters — keep them separate
            continue
        elif ca is not None:
            clusters[ca].add(b)
    ...
    for cid, members in sorted(clusters.items()):
        ...
        result.append(FileCluster(
            ...
            shared_importers=[],  # populated below
            score=avg,
        ))
...
def _find_shared_importers(
    cluster: FileCluster,
    imported_by: dict[str, list[str]],
) -> list[str]:
    """Find files that import >=2 members of this cluster."""
...
def _find_import_chains(
    edges: dict[str, list[str]],
    max_length: int = 6,
) -> list[ImportChain]:
    """Find longest import chains (A imports B imports C ...).

    Uses DFS from each file, tracking visited to avoid cycles.
    """
    ...
    # Sort by length descending, take unique top chains
...
def _find_integration_points(
    ...
    """Find files that bridge two or more clusters (integration seams)."""
    file_to_cluster: dict[str, int] = {}
    for cl in clusters:
        for f in cl.files:
...
def build_clusters(graph: ImportGraph) -> ClusterResults:
    """Build co-import clusters from the import graph.

    Args:
        graph: The import graph with edges and importedBy map.

    Returns:
        ``ClusterResults`` with clusters, chains, and integration points.
    """
    scores = _compute_co_import_scores(graph.imported_by)
    raw_clusters = _greedy_cluster(scores)

    # Populate shared importers for each cluster
    for cluster in raw_clusters:
        cluster.shared_importers = _find_shared_importers(
            cluster, graph.imported_by,
        )

    # Sort by average score descending, cap
