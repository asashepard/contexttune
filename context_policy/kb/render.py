...
Each renderer takes structured probe data and produces a text block
with line-based budget enforcement.

Line budgets:
- architecture: 200 lines
- symbol_map: 300 lines
- context: 200 lines

Item caps:
- hubs: 12, hub details: 3, entry points: 10, directories: 12
- data models: 15, symbols per file: 10, files in symbol index: 30
- clusters: 6, chains: 8, integrations: 4
"""
...
def _cap_lines(text: str, budget: int) -> str:
    """Truncate text to at most *budget* lines."""
...
def render_architecture(graph: ImportGraph, entries: EntryPoints) -> str:
    """Render the architecture section from import graph and entry points.

    Includes hub modules table (in-degree, top importers, key exports)
    and entry point table (file, kind, classification, confidence).
    """
    parts: list[str] = []
...
def render_symbol_map(index: SymbolIndex) -> str:
    """Render the symbol map section.

    Groups symbols by file, rendering each as a table with
    Symbol | Kind | Signature | Used In columns.
    """
    parts: list[str] = []
    file_count = 0
...
def render_context(clusters: ClusterResults, tests: TestInfo) -> str:
    """Render the context section from clusters and test info.

    Includes co-import clusters, import chains, integration seams,
    and test infrastructure details.
    """
    parts: list[str] = []
    ...
    if clusters.clusters:
        ...
        for cl in clusters.clusters[:6]:
            ...
            parts.append(
                f"- **Cluster {cl.id}** (score {cl.score:.1f}): "
                f"{files_str}"
            )
            if importers_str:
    ...
    if clusters.integrations:
        ...
        parts.append(
            "Files that bridge multiple clusters "
            "(editing these has broad impact).\n"
        )
    ...
    if tests.conftest_paths:
        parts.append(
            f"- Conftest files: {', '.join(tests.conftest_paths[:6])}"
        )
    if tests.fixtures:
...
def render_conventions(conv: Conventions) -> str:
    ...
    if conv.type_hint_prevalence > 0:
        parts.append(
            f"- Type hint coverage: {conv.type_hint_prevalence:.0%} of functions"
        )
